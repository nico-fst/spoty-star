...
def subtract_uris_existing_in_playlist(
    playlist_name: str, track_uris: List[str]
) -> bool:
    ...
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        all_tracks = list(
            executor.map(
                lambda offset: thread_spotify_get_tracks(
                    tracks_href, offset, limit, access_token
                ),
                offsets,
            )
        )
