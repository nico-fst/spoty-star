...
def thread_get_favs_chunk(
    limit: int, offset: int, access_token: str, datetime_start=None, datetime_end=None
):
    ...
    if datetime_start or datetime_end:
        ...
        for item in items:
            ...
            if (not datetime_start or added_at >= datetime_start) and (
                not datetime_end or added_at <= datetime_end
            ):
...
def get_favs(date_start: str = None, date_end: str = None) -> List[FavEntry]:
    ...
    tracks_total = spotify_get("https://api.spotify.com/v1/me/tracks?limit=1").json()[
        "total"
    ]
    ...
    with ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        favs_list_list = list(
            executor.map(
                lambda offset: thread_get_favs_chunk(
                    limit, offset, access_token, datetime_start, datetime_end
                ),  # function
                offsets,  # iterable
            )
        )
    ...
    favs = [
        song for result in favs_list_list for song in result
    ]  # weil List[List[songs]]
