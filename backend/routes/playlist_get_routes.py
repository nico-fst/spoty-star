...
from ..exceptions import ClientError, NotFoundError, LoggedOutError
...
def get_playlists() -> List[Playlist]:
    """returns user's playlists or [] if none"""
    ...
    offsets = list(
        range(0, total_playlists, 50)
    )  # Spotify API allows max 50 per request
    ...
    with ThreadPoolExecutor(max_workers=40) as executor:
        playlists_list_list = list(
            executor.map(
                lambda offset: thread_get_playlists(50, offset, access_token).json()[
                    "items"
                ],
                offsets,
            )
    ...
    playlists = [
        playlist for result in playlists_list_list for playlist in result
    ]  # weil List[List[playlists]]
    return playlists
...
def get_playlist(playlist_name: str) -> Playlist:
    ...
    playlist_in_dict = [
        pl for pl in playlists if pl["name"].lower() == playlist_name.lower()
    ]
