...
@app.route('/login')
def login():
    auth_url = f"{AUTH_URL}?response_type=code&client_id={CLIENT_ID}&scope={SCOPE}&redirect_uri={REDIRECT_URL}"
    return redirect(auth_url)
    
@app.route('/callback')
def callback():
    print("IN CALLBACK ANGEKOMMEN")
...
def currently_playing():
    ...
    if resp.status_code == 200 and resp.json():
        ...
        return f'Aktuell läuft: {track_name} von {track_artist_name}'
    else:
